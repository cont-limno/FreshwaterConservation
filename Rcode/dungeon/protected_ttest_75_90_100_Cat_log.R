# performs t-test between protected and unprotected lakes at 
# thresholds of 75, 90 and 100 percent local catchment (Cat) protected (for consideration as "protected")

protected_ttest_75_90_100_Cat_log <- function(dataframe, yvar){
  ...
  pro <- log(subset(dataframe, ProtectGAP12Cat_75=='Protected75')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP12Cat_75=='Unprotected75')[,yvar])
  ...
  pro <- log(subset(dataframe, ProtectGAP12Cat_90=='Protected90')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP12Cat_90=='Unprotected90')[,yvar])
  ...
  pro <- log(subset(dataframe, ProtectGAP12Cat_100=='Protected100')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP12Cat_100=='Unprotected100')[,yvar])
  ...
  # 75% threshold for counting as protected
  pro <- log(subset(dataframe, ProtectGAP123Cat_75=='Protected75')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP123Cat_75=='Unprotected75')[,yvar])
  var1 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  ...
  # 90% threshold for counting as protected
  pro <- log(subset(dataframe, ProtectGAP123Cat_90=='Protected90')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP123Cat_90=='Unprotected90')[,yvar])
  var2 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  ...
  # 100% threshold for counting as protected
  pro <- log(subset(dataframe, ProtectGAP123Cat_100=='Protected100')[,yvar])
  unpro <- log(subset(dataframe, ProtectGAP123Cat_100=='Unprotected100')[,yvar])
  var3 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  if (var3$p.value > 0.05) {
