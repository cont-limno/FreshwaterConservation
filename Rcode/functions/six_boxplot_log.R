...
# desired variable log transformed

six_boxplot_log <- function(dataframe, yvar, ylimits){
  ...
  boxplot(log(value) ~ level, data=test, las=1, col=c('green','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[1], ylab=paste0('log ',yvar), xlab='GAPS 1-2')
  boxplot(log(value) ~ level, data=test2, las=1, col=c('green','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[2], xlab='GAPS 1-2')
  boxplot(log(value) ~ level, data=test3, las=1, col=c('green','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[3], xlab='GAPS 1-2')
  boxplot(log(value) ~ level, data=test4, las=1, col=c('dodgerblue','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[1], ylab=paste0('log ',yvar), xlab='GAPS 1-3')
  boxplot(log(value) ~ level, data=test5, las=1, col=c('dodgerblue','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[2], xlab='GAPS 1-3')
  boxplot(log(value) ~ level, data=test6, las=1, col=c('dodgerblue','gray'), ylim=ylimits, names=xlabz, outline=T, main=titlez[3], xlab='GAPS 1-3')
