# function to perform t tests at local catchment protection thresholds of 75,90 and 100% (to be considered protected)
protected_ttest_75_90_100_Cat <- function(dataframe, yvar){
  ...
  pro <- subset(dataframe, ProtectGAP12Cat_90=='Protected90')[,yvar]
  unpro <- subset(dataframe, ProtectGAP12Cat_90=='Unprotected90')[,yvar]
  ...
  pro <- subset(dataframe, ProtectGAP12Cat_100=='Protected100')[,yvar]
  unpro <- subset(dataframe, ProtectGAP12Cat_100=='Unprotected100')[,yvar]
  ...
  # 75% threshold for counting as protected
  pro <- subset(dataframe, ProtectGAP123Cat_75=='Protected75')[,yvar]
  unpro <- subset(dataframe, ProtectGAP123Cat_75=='Unprotected75')[,yvar]
  var1 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  ...
  # 90% threshold for counting as protected
  pro <- subset(dataframe, ProtectGAP123Cat_90=='Protected90')[,yvar]
  unpro <- subset(dataframe, ProtectGAP123Cat_90=='Unprotected90')[,yvar]
  var2 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  ...
  # 100% threshold for counting as protected
  pro <- subset(dataframe, ProtectGAP123Cat_100=='Protected100')[,yvar]
  unpro <- subset(dataframe, ProtectGAP123Cat_100=='Unprotected100')[,yvar]
  var3 <- var.test(pro,unpro) #if p > 0.05, have homoskedasticity
  if (var3$p.value > 0.05) {
