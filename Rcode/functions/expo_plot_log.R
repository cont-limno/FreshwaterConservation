...
# with log transformation built in for predictor variable (xvar)
expo_plot_log <- function(xvar, yvar, dataframe, min_protected){
  ...
  plot(data_sub[,yvar] ~ log(data_sub[,xvar]), data=data_sub, pch=20, xlab=xvar, ylab=yvar)
  plot_cor <- round(cor(data_sub[,yvar], log(data_sub[,xvar]), use='pairwise.complete.obs'),3)
  plot_lm <- lm(data_sub[,yvar] ~ log(data_sub[,xvar]), data=data_sub)
